...
for (i in (1:K)){
  ...
  nngp <- function(y,y.ho,coords,coords.ho,phi,sigma.sq,tau.sq,n.samples = 1000,m=10){
    ...
    list("mse" = mse, "prediction" = p.r, "m" = m)
  }
  ...
  print(list("iter" = i, "gpgp" = mse1,"quadN" = mse2))
  results1[i,] <- c(mse1,mse2)
}
write.csv(results1,"exponential_users.csv",row.names = F)
